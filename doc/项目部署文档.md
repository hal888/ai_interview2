...
## 1. 项目概述
Offer贝是一个基于AI技术的面试辅助系统，旨在帮助用户准备面试，提高面试成功率。系统提供简历优化、自我介绍生成、智能题库、模拟面试和面试策略等功能。
...
### 3.1 克隆代码仓库
```bash
git clone https://github.com/hal888/offerBei.git
cd offerBei
```
...
#### 3.3.1 环境变量配置
项目使用 `.env` 文件进行配置管理。请复制 `.env.example` 文件并重命名为 `.env`，然后填入实际配置信息。
...
创建Nginx配置文件 `/etc/nginx/conf.d/offerBei.conf`：

```nginx
server {
    listen 80;
    server_name https://offerbei.ailongdev.com/;

    # 前端静态文件
    location / {
        root /var/www/offerBei/dist;
        index index.html;
